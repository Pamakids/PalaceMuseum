...
//			trace(sound is Sound);
		 ...
		 */
		public function play(target:Object, startPosition:int=0, _vol:Number=-1):SoundChannel
		{
			var s:Object;
			var o:Object;
			if (target is String)
			...
			if (s)
			{
				...
				if (_vol >= 0)
				{
					var ts:SoundTransform=new SoundTransform(_vol);
					sc.soundTransform=ts;
				}
				sc.addEventListener(Event.SOUND_COMPLETE, playedHandler);
				playingSounds[target]={channel: sc, sound: s, volume: _vol};
				return sc;
			}
			else
			{
