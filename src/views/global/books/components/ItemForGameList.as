	...
	public class ItemForGameList extends FeathersControl
	{
		...
		*/
		private function udpateStars():void
		{
			...
			{
				_stars[i].texture = (i<count)?BooksManager.getTexture("icon_star_red"):BooksManager.getTexture("icon_star_gray");
			}
		}
		/*
		* 游戏图标按钮
		*/
		private function initButton():void
		{
			...
			gameIcon.width = 205;
			gameIcon.height = 159;
			gameIcon.defaultSkin = BooksManager.getImage("card_game_"+_data.iconIndex);
			...
			if(!isOpend)
			{
				...
				mask.x = 7;
				mask.y = 7;
				lock = BooksManager.getImage("icon_lock");
				lock.x = 15;
				lock.y = 96;
				this.addChild( lock );
		...
		{
			var texture:Texture = BooksManager.getTexture("line_short");
			...
			texture = BooksManager.getTexture("icon_star_gray");
			for(var i:int = 0;i<max;i++)
			{
